...
for w in players_bvp:
    ...
    if beans:  # Only proceed if a matching player is found
        ...
        for a in beans.get('stats'):
            ...
            fhrs_per_game = str(Fraction(round((hrs / games_played), 2)).limit_denominator(10))
            hits_per_game = round((hits / games_played), 3)
            fhits_per_game = str(Fraction(round((hits / games_played), 2)).limit_denominator(10))
            rbis_per_game = round((rbi / games_played), 3)
            frbis_per_game = str(Fraction(round((rbi / games_played), 2)).limit_denominator(10))
            w.append(int(hrs))
