  ...
  const App = {
    ...
    async loadIntoModal(url) {
      ...
      try {
        ...
        // Simple heuristic: if it looks like HTML, inject as HTML; otherwise preserve whitespace
    ...
    updateStickTop() {
      ...
      // Height of the modal bar, consumed by thead { top: ... }
      body.style.setProperty('--stick-top', h + 'px');
      // Small visual gap between the bar and the sticky table header
      // Tweak this value (px) to increase/decrease the offset.
      body.style.setProperty('--stick-gap', '8px');
    },
