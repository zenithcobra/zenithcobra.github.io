...
def _extract_highlight_link(
    highlights_list: List[str], keyword: str, link_text: str
) -> Optional[str]:
    ...
    for i, line in enumerate(highlights_list):
        if keyword in line and i + 2 < len(highlights_list):
            ...
            return re.sub(
                url_pattern, rf'<a href="\1" target="_blank">{link_text}</a>', link_url
            )
    return None
...
def predict_streak_continuation(
    current_streak: Tuple[int, int], stats: Dict[str, float]
) -> int:
    """
...
def analyze_binary_list(binary_list: List[int]) -> Dict[str, int]:
    ...
    return {"trials": trials, "success": success, "failures": failures}
...
def analyze_streaks(streaks: List[Dict[str, int]]) -> Dict[str, float]:
    ...
    return {
        ...
        "average_win_streak_length": (
            sum(win_streaks) / len(win_streaks) if win_streaks else 0
        ),
        "number_of_lose_streaks": len(lose_streaks),
        "longest_lose_streak": max(lose_streaks) if lose_streaks else 0,
        "average_lose_streak_length": (
            sum(lose_streaks) / len(lose_streaks) if lose_streaks else 0
        ),
    }
...
def get_teams_playing_today_from_processed_schedule(
    processed_schedule: List[Dict[str, Any]],
) -> List[Dict[str, str]]:
...
def get_team_history(
    teams_list: List[Dict[str, Any]], games_back: int = 10
) -> List[Dict[str, Any]]:
    ...
    for team in teams_list:
        ...
        start_date = end_date - timedelta(
            days=30
        )  # Look back 30 days to get enough games
        ...
        try:
            ...
            last_games = (
                completed_games[-games_back:]
                if len(completed_games) >= games_back
...
def get_team_records(teams_history: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for team in teams_history:
            ...
            try:
                ...
                if not winning_team:
                    ...
                    if away_score > home_score:
                        ...
                        print(
                            f"Warning: Unable to determine winner for game_id {game_id}"
                        )
                        continue
                ...
                if winning_team == team_name:
                    team_record += "W-"
                    vs_team = game_data.get(
                        "losing_team",
                        (
                            game_data.get("home_name")
                            if game_data.get("away_name") == team_name
        ...
        team.update({"team_record": team_record, "team_record_plus": list_of_results})
    ...
    for team in team_data:
        ...
        if binary_record:
            ...
            stats = {
                ...
                "average_win_streak_length": streak_stats.get(
                    "average_win_streak_length", 0
                ),
                "longest_lose_streak": streak_stats.get("longest_lose_streak", 0),
                "average_lose_streak_length": streak_stats.get(
                    "average_lose_streak_length", 0
                ),
            }

            predicted_outcome = predict_streak_continuation(
                (current_streak, amount), stats
            )
            ...
            team.update(
                {
                    ...
                    "wl_mmpp": streak_stats.get(
                        (
                            "average_win_streak_length"
                            if current_streak == 1
                            else "average_lose_streak_length"
                        ),
                        0,
                    ),
                }
            ...
            team.update({"prediction": "Unknown", "wl_mmp": "No data", "wl_mmpp": 0})
...
def analyze_sequence_and_predict(data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in data:
        ...
        item.update({"prediction": "Unknown", "hr_mmp": "No data", "hr_mmpp": 0})

    return data


def get_yesterdays_homers(
    batters_with_streaks: List[Dict[str, Any]],
) -> List[Dict[str, Any]]:
    ...
    for game in yschedule:
        ...
        processed_plays = [
            line.split(")")[0] + ")" for line in filtered_plays if ")" in line
        ]
    ...
    for player_name in new_homers:
        try:
            ...
            player_stats = statsapi.player_stat_data(
                next(
                    ...
                    for x in statsapi.get(
                        "sports_players",
                        {"season": str(config.CURRENT_SEASON), "gameType": "W"},
                    )["people"]
                    if x["fullName"] == player_name
                ),
                "hitting",
                "season",
            )
            ...
            if player_stats:
                ...
                player_data.update({"team": team_name, "player_id": player_id})
