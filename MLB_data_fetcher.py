...
def get_bvp_stats(batter_id: int, pitcher_id: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        stats_data = mlb.get_player_stats(
            batter_id, stats=stats, groups=group, **params
        )
...
def get_yesterdays_report(date: Optional[str] = None) -> List[str]:
    ...
    entries_with_toronto = [
        x for x in yschedule if "summary" in x and "Toronto" in x["summary"]
    ]
    entries_without_toronto = [
        x for x in yschedule if not ("summary" in x and "Toronto" in x["summary"])
    ]
...
def _process_game_report(
    game: Dict[str, Any], include_highlights: bool = False
) -> List[str]:
    ...
    if include_highlights:
        ...
        highlights_with_links = re.sub(
            url_pattern, r'<a href="\1" target="_blank">video link</a>', highlights
        )
        ...
        condensed_link = _extract_highlight_link(
            highlights_list, "Condensed", "Condensed Game"
        )
        video_link = _extract_highlight_link(
            highlights_list, "Highlights", "Highlights Video"
        )
